...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Doara.Api.Web` and `Doara.Api.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 574a87d1-b5d1-460b-b051-c01e17b59867
```

> `574a87d1-b5d1-460b-b051-c01e17b59867` is the password of the certificate, you can change it to any password you want.
...
Run `Doara.Api.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Doara.Api.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Doara.Api.Web`: ASP.NET Core MVC / Razor Pages application that is the essential web application of the solution.
